...
import {boolean, integer, number, string, stringGlob} from '..'

config({path: join(__dirname, './example.env')})

test(`string() with a single key should resolve properly`,
    () => expect(string('EXAMPLE_STRING', 'Bye world!')).toBe('Hello world!')
)

test(`string() with multiple keys should resolve properly`,
...
test('`stringGlob()` works properly', () => {
    interface Vars {
        EXAMPLE_STRING: string,
        EXAMPLE_INVALID_BOOLEAN: string
    }
    const vars = <Vars>stringGlob('EXAMPLE_*')
    expect(vars.EXAMPLE_STRING).toBe('Hello world!')
})
